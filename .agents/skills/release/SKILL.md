...
5. `main` がリリース対象なら、現在の未コミット変更を release 対象として扱う。検証が通った後、version bump、lab 記録、docs、tests、skill、generated views、lock 更新を含めて作業 branch に commit し、PR を作成して required checks を確認してから merge する。`main` へ直接 push しない。dirty worktree のまま既存 `main` を release しない。
