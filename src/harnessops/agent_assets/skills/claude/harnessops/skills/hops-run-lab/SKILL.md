...
実装や判断の前には `hops lab context` で関連 dossier、過去判断、guard、knowledge を取り出す。作業選定では `hops lab queue` を使い、停滞検出では `hops lab lifecycle lint --warn-only` を使う。記録を増やすだけで終えず、次の行動、評価、guard、忘却候補へ接続する。
